...
public class ELServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//JSP로 포워딩 할 때 request에 담아서 보내줬음! => Servlet내장 객체 /Scope객체
		//application / session / request/page
		
		/*범위에 대한 : 자바타입
		 * 
		 * 1.application scope : ServletContext type
		 * 한 애플리케이션(프로젝트)당 단 한 개 존재하는 객체
		 * 이 영역에 데이터을 담으면 애플리케이션 전역에서 사용 가능
		 * 2.session scope : HttpSession
		 * 하나의 브라우저 당 한 개 존재한는 객체 
		 ...
		 * 이 영역에 데이터를 담으면 request객체를 포워딩 받는 응답 JSP에서만 사용가능 (1회성)
		 * 4. page scope: PageContext
		 * JSP페이지 내에 서만 사용가능 
		 * 
		 * => 위 객체들에 속성을 추가할 때 는 .setAttribute("키" , 벨류);
		 * 							 .getAttribute("키");
		 * 					    	 .removeAttribute("키");
		 */
		//resquestScope에 먼저 담아보자
		request.setAttribute("classRoom","A강의장");
		request.setAttribute("student", new Person("홍길동",15,"한양"));
		
		
		//sessionScope
		HttpSession session =request.getSession(); //get을 통애  먼저 session을 얻어옴
		...
		request.setAttribute("key", "sessin key");
		...
		//여기서 저 숫자는 db에서 수행한 겨 ㄹ과
		
		//문자열
		request.setAttribute("strOne","안녕");
		request.setAttribute("strTwo", new String("하이룽"));
